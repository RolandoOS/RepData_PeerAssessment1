...
```{r}
p <- ggplot(dat_2, aes(x=interval, y=steps)) +
...
p <- ggplot(dat_comp, aes(x=daily, color=group)) +
...
```{r}
p <- ggplot(dat_w, aes(interval, y=steps, color=Weekend)) +
    geom_line(size=0.5) +
    theme_minimal() +
